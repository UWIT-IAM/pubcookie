...
/*
    $Id: candv.c,v 1.23 2004-12-22 22:14:54 willey Exp $
 */
...
int main (int argc, char **argv)
{
    ...
    cookie =
        libpbc_get_cookie ((unsigned char *) user, type, creds,
                           pre_sess_token, appsrvid, appid, s_ctx_plus,
                           c_stuff);

    if (!
        (cookie_data =
         libpbc_unbundle_cookie ((char *) cookie, v_ctx_plus, c_stuff))) {
    ...
    cookie_data2 =
        libpbc_unbundle_cookie ((char *) updated_cookie, v_ctx_plus,
                                c_stuff);
    if (cookie_data2) {
